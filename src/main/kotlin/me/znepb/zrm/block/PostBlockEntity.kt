...
open class PostBlockEntity(pos: BlockPos, state: BlockState) : BlockEntity(Registry.ModBlockEntities.POST_BLOCK_ENTITY, pos, state) {
    ...
    fun getPlacementState(pos: BlockPos) {
        ...
        wasPlacementSuccess = true
        if(stateDown == null || stateUp == null || stateNorth == null || stateEast == null || stateSouth == null || stateWest == null) wasPlacementSuccess = false
        if(stateDown?.block == Blocks.VOID_AIR || stateUp?.block == Blocks.VOID_AIR || stateNorth?.block == Blocks.VOID_AIR
            || stateEast?.block == Blocks.VOID_AIR || stateSouth?.block == Blocks.VOID_AIR || stateWest?.block == Blocks.VOID_AIR)  wasPlacementSuccess = false
    }
